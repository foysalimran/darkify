 ...
 *
 */
if ( ! class_exists( 'DRK_Field_wp_editor' ) ) {
  class DRK_Field_wp_editor extends DRK_Fields {
    ...
    public function enqueue() {
      ...
      if ( drk_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var drk_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( drk_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'drk_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'drk_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'drk_wp_editor', $setup );
