 ...
 *
 */
if ( ! class_exists( 'DRK_Field_dimensions' ) ) {
  class DRK_Field_dimensions extends DRK_Fields {
    ...
    public function render() {
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo '<div class="drk--input">';
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="drk--label drk--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="drk-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="drk--label drk--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="drk--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="drk--label drk--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="drk-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="drk--label drk--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
