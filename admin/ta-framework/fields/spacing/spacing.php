 ...
 *
 */
if ( ! class_exists( 'DRK_Field_spacing' ) ) {
  class DRK_Field_spacing extends DRK_Fields {
    ...
    public function render() {
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo ( ! empty( $args['all_icon'] ) ) ? '<span class="drk--label drk--icon">'. $args['all_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="drk-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="drk--label drk--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<div class="drk--input">';
          echo ( ! empty( $args[$property.'_icon'] ) ) ? '<span class="drk--label drk--icon">'. $args[$property.'_icon'] .'</span>' : '';
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="drk-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="drk--label drk--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
