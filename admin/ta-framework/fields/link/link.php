 ...
 *
 */
if ( ! class_exists( 'DRK_Field_link' ) ) {
  class DRK_Field_link extends DRK_Fields {
    ...
    public function render() {
      ...
      echo '<div class="'. esc_attr( $maybe_hidden ) .'"><div class="drk--result">'. sprintf( '{url:"%s", text:"%s", target:"%s"}', $value['url'], $value['text'], $value['target'] ) .'</div></div>';

      echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[url]' ) ) .'" value="'. esc_attr( $value['url'] ) .'"'. $this->field_attributes( array( 'class' => 'drk--url' ) ) .' />';
      echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[text]' ) ) .'" value="'. esc_attr( $value['text'] ) .'" class="drk--text" />';
