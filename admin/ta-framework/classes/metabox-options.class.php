 ...
 *
 */
if ( ! class_exists( 'DRK_LITE_Metabox' ) ) {
  class DRK_LITE_Metabox extends DRK_LITE_Abstract{
    ...
    public function add_metabox_classes( $classes ) {
      ...
      if ( ! empty( $this->post_formats ) ) {
        ...
        foreach ( $this->post_formats as $format ) {
          $classes[] = 'drk_lite-post-format-'. $format;
        }

        if ( ! in_array( $saved_post_format, $this->post_formats ) ) {
          $classes[] = 'drk_lite-metabox-hide';
        } else {
          $classes[] = 'drk_lite-metabox-show';
        }
      ...
      if ( ! empty( $this->page_templates ) ) {
        ...
        $classes[] = 'drk_lite-page-templates';

        foreach ( $this->page_templates as $template ) {
          $classes[] = 'drk_lite-page-'. preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
        }
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors    = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_drk_lite_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $theme     = ( $this->args['theme'] ) ? ' drk_lite-theme-'. $this->args['theme'] : '';
      $nav_type  = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_drk_lite_errors_'. $this->unique );
      }

      wp_nonce_field( 'drk_lite_metabox_nonce', 'drk_lite_metabox_nonce'. $this->unique );

      echo '<div class="drk_lite drk_lite-metabox'. esc_attr( $theme ) .'">';

        echo '<div class="drk_lite-wrapper'. esc_attr( $show_all ) .'">';

          if ( $has_nav ) {

            echo '<div class="drk_lite-nav drk_lite-nav-'. esc_attr( $nav_type ) .' drk_lite-nav-metabox">';
              ...
              foreach ( $this->sections as $section ) {
                ...
                $tab_error = ( ! empty( $errors['sections'][$tab_key] ) ) ? '<i class="drk_lite-label-error drk_lite-error">!</i>' : '';
                $tab_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="drk_lite-tab-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
          ...
          echo '<div class="drk_lite-content">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              echo '<div class="drk_lite-section hidden'. esc_attr( $section_onload . $section_class ) .'">';

              echo ( $section_title || $section_icon ) ? '<div class="drk_lite-section-title"><h3>'. wp_kses_post($section_icon) . esc_html($section_title) .'</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  DRK_LITE::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
    ...
    public function save_meta_box( $post_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'drk_lite_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'drk_lite_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $data = apply_filters( "drk_lite_{$this->unique}_save", $data, $post_id, $this );

      do_action( "drk_lite_{$this->unique}_save_before", $data, $post_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_drk_lite_errors_'. $this->unique, $errors );
        }

      }

      do_action( "drk_lite_{$this->unique}_saved", $data, $post_id, $this );

      do_action( "drk_lite_{$this->unique}_save_after", $data, $post_id, $this );
