...
if ( ! class_exists( 'DRK_LITE_Setup' ) ) {
	class DRK_LITE_Setup {
		...
		public static function textdomain() {
			load_textdomain( 'chat-skype', self::$dir . '/languages/' . get_locale() . '.mo' );
		}
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( apply_filters( 'drk_lite_fa4', false ) ) {
				wp_enqueue_style( 'drk_lite-fa4', DRK_LITE_DIR_URL . 'admin/assets/css/font-awesome.min.css', array(), '4.7.0', 'all' );
			} else {
				wp_enqueue_style( 'drk_lite-fa5', DRK_LITE_DIR_URL . 'admin/ta-framework/assets/css/fontawesome-free@5.15.4_css_all.min.css', array(), '5.15.5', 'all' );
				wp_enqueue_style( 'drk_lite-fa5-v4-shims', DRK_LITE_DIR_URL . 'admin/ta-framework/assets/css/fontawesome-free@5.15.4_css_v4-shims.min.css', array(), '5.15.5', 'all' );
			}

			// Main style
			wp_enqueue_style( 'chat-skype', self::include_plugin_url( 'assets/css/style' . $min . '.css' ), array(), self::$version, 'all' );
			...
			wp_enqueue_script( 'chat-skype', self::include_plugin_url( 'assets/js/main' . $min . '.js' ), array( 'drk_lite-plugins' ), self::$version, true );
			...
			wp_localize_script(
				...
				array(
					...
					'i18n'          => array(
						'confirm'         => esc_html__( 'Are you sure?', 'chat-skype' ),
						'typing_text'     => esc_html__( 'Please enter %s or more characters', 'chat-skype' ),
						'searching_text'  => esc_html__( 'Searching...', 'chat-skype' ),
						'no_results_text' => esc_html__( 'No results found.', 'chat-skype' ),
					),
				)
			);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = esc_html__( 'Oops! Not allowed.', 'chat-skype' ) . ' <strong>(' . $field_type . ')</strong>';
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			}
			...
			echo wp_kses_post( '<div class="drk_lite-field drk_lite-field-' . esc_attr( $field_type ) . esc_attr( $is_pseudo ) . esc_attr( $class ) . esc_attr( $visible ) . '"' . wp_kses_data( $depend ) . '>' );
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( ! empty( $field['title'] ) ) {
					...
					echo ( ! empty( $field['subtitle'] ) ) ? '<div class="drk_lite-subtitle-text">' . esc_html( $field['subtitle'] ) . '</div>' : '';
					echo '</div>';
				}
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'Field not found!', 'chat-skype' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'Field not found!', 'chat-skype' ) . '</p>';
			}
...
DRK_LITE_Setup::init( __FILE__, false );
