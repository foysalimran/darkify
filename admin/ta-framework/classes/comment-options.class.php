 ...
 *
 */
if ( ! class_exists( 'DRK_LITE_Comment_Metabox' ) ) {
  class DRK_LITE_Comment_Metabox extends DRK_LITE_Abstract{
    ...
    public function add_comment_meta_box_content( $comment, $callback ) {
      ...
      $errors   = ( is_object ( $comment ) ) ? get_comment_meta( $comment->comment_ID, '_drk_lite_errors_'. $this->unique, true ) : array();
      $errors   = ( ! empty( $errors ) ) ? $errors : array();
      $theme    = ( $this->args['theme'] ) ? ' drk_lite-theme-'. $this->args['theme'] : '';
      $nav_type = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object( $comment ) && ! empty( $errors ) ) {
        delete_comment_meta( $comment->comment_ID, '_drk_lite_errors_'. $this->unique );
      }

      wp_nonce_field( 'drk_lite_comment_metabox_nonce', 'drk_lite_comment_metabox_nonce'. $this->unique );

      echo '<div class="drk_lite drk_lite-comment-metabox'. esc_attr( $theme ) .'">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  DRK_LITE::field( $field, $this->get_meta_value( $comment->comment_ID, $field ), $this->unique, 'comment_metabox' );
    ...
    public function save_comment_meta_box( $comment_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'drk_lite_comment_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'drk_lite_comment_metabox_nonce' ) ) {
        return $comment_id;
      }
      ...
      $data = apply_filters( "drk_lite_{$this->unique}_save", $data, $comment_id, $this );

      do_action( "drk_lite_{$this->unique}_save_before", $data, $comment_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_comment_meta( $comment_id, '_drk_lite_errors_'. $this->unique, $errors );
        }

      }

      do_action( "drk_lite_{$this->unique}_saved", $data, $comment_id, $this );

      do_action( "drk_lite_{$this->unique}_save_after", $data, $comment_id, $this );
